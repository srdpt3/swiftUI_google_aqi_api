...
//  DSAQIndex
//
//  Created by Dustin Yang on 8/1/24.
//
...
struct CircleAQIView : View {
    ...
    var body: some View {
        Circle().stroke(Color(red: aqi.color.red, green: aqi.color.green, blue: aqi.color.blue), lineWidth: isSelected ? 4 : 3)
            .frame(width: size.width, height: size.height)
            .overlay{
                Text(aqi.aqiDisplay).foregroundStyle(.white).fontWeight(.bold)
            }
